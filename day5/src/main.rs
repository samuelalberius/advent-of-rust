fn main() {
    let (crates_str, commands_str) = include_str!("input.txt")
        .split_once("\n\n")
        .unwrap();
    ...
    crates_str
        ...
        .for_each(|line| line.chars()
            ...
            .for_each(|(i, char)| {
                crates[(i as f64 / 4.0).floor() as usize].push(char);
            })
        );
    ...
    commands_str
        .lines()
        .for_each(|line| {
            let t: Vec<usize> = line
                .split_ascii_whitespace()
                .filter_map(|s| s.parse::<usize>().ok())
                .collect();
            commands.push((t[0], t[1], t[2]));
            }
        );
